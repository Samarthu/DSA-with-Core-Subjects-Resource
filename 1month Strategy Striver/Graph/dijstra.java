// Dijstra's Algorithm Of shortest Path
import java.util.*;
class Edge{
	int s;
	int nbr;
...
class dijstra{
   ...
   static void dijstra(ArrayList<Edge>[] graph, int src,int wt,boolean[] visited )
   {
   	   PriorityQueue<pair> q = new PriorityQueue<>();
	   q.add(new pair(src,wt,src+""));
	   while(q.size() > 0){
	   	  // r m* w a*
	   	  pair rem=q.poll();
	   	  ...
	   	  for(Edge e : graph[rem.v])
	   	  {
	   	  	  ...
	   	  	  {
	   	  	     q.add(new pair(e.nbr,rem.wt + e.wt,rem.psf+e.nbr));
	   	  	  }
	   	  }
	   }
   }
